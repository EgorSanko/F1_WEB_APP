...
RUSTORE_URL = "https://www.rustore.ru/catalog/app/ru.leadseek.f1hub"
UPDATE_TITLE = "ОБНОВИТЕ ПРИЛОЖЕНИЕ"
UPDATE_TEXT = "Вышла версия 2.0 — откройте RuStore и нажмите «Обновить»"


def _is_legacy_app(request) -> bool:
    """LEGACY_WALL: старая нативка 1.0.x из RuStore. У неё UA okhttp/expo,
    у новой оболочки 2.0 — F1HubApp/2.0 (её не трогаем)."""
    try:
    ...
    if "f1hubapp" in ua:
        return False
    return "okhttp" in ua or "expo/" in ua


def _wall_home(data):
    """Подменяем ТОЛЬКО строки, схему оставляем как есть — иначе приложение падает."""
    try:
        out = dict(data)
        nr = dict(out.get("next_race") or {})
        if nr:
            nr["name"] = UPDATE_TITLE
            nr["circuit"] = UPDATE_TEXT
            nr["circuit_name"] = UPDATE_TEXT
            nr["country"] = "RuStore → F1 Hub → Обновить"
            nr["locality"] = "Версия 2.0"
            out["next_race"] = nr
        lr = dict(out.get("last_race") or {})
        if lr:
            lr["name"] = UPDATE_TITLE
            lr["results"] = []
            out["last_race"] = lr
        st = out.get("standings_top3")
        if isinstance(st, dict):
            out["standings_top3"] = {**st, "standings": []}
        out["upgrade_required"] = True
        out["upgrade_url"] = RUSTORE_URL
        return out
    except Exception:
        return data


def _wall_list(data, key, item):
    """Первым элементом списка кладём призыв со ссылкой — по нему открывается RuStore."""
    try:
...
async def get_home(request: Request, season: int = None):
    # Старая нативка 1.0.x (UA okhttp/expo, зовёт /api/home БЕЗ ?season=) получает
    # ОБЫЧНЫЕ данные. Жёсткую заглушку убрали: приложение её не разбирало и не
    # открывалось вовсе, а обновиться было некуда — 2.0 ещё не в сторе.
    # Призыв обновиться показываем мягко — баннером в ленте новостей (см. /api/news).
    season = season or CURRENT_SEASON
    """Combined endpoint: next race + last race + top standings. One call for home screen."""
    data = await f1_data.get_home_data(season=season)
    if _is_legacy_app(request):
        return _wall_home(data)
    return data
...
async def prediction_rules():
    """Сколько очков даёт каждый тип прогноза. Публично: нужно сайту,
    приватных данных не содержит. Значения — из config.PREDICTION_POINTS."""
    P = PREDICTION_POINTS
    return {"rules": [
        {"type": "podium", "label": "Подиум",
         "max_points": P["podium"]["exact_order"],
         "description": f"Все трое на своих местах — {P['podium']['exact_order']}. "
                        f"Тройка угадана, порядок другой — {P['podium']['all_3']}. "
                        f"Двое — {P['podium']['2_of_3']}, один — {P['podium']['1_of_3']}."},
        {"type": "fastest_lap", "label": "Быстрый круг",
         "max_points": P["fastest_lap"]["correct"],
         "description": "Пилот, показавший лучшее время круга в гонке. Угадать сложнее победителя — и стоит дороже."},
        {"type": "winner", "label": "Победитель гонки",
         "max_points": P["winner"]["correct"],
         "description": "Кто первым пересечёт финишную черту."},
        {"type": "dnf_count", "label": "Число сходов",
         "max_points": P["dnf_count"]["exact"],
         "description": f"Точное число сошедших — {P['dnf_count']['exact']}, "
                        f"ошибка на одного — {P['dnf_count']['off_by_1']}."},
        {"type": "safety_car", "label": "Сейфти-кар",
         "max_points": P["safety_car"]["correct"],
         "description": "Выедет ли машина безопасности по ходу гонки."},
        {"type": "top10", "label": "Десятка",
         "max_points": P["top10"]["exact_position"],
         "description": f"За точную позицию — {P['top10']['exact_position']}, "
                        f"за попадание в десятку — {P['top10']['in_top10']}."},
    ]}
...
def _with_legacy_upgrade_banner(response, request):
    """legacy_upgrade_banner: старому приложению из RuStore (UA okhttp/expo)
    первым постом в ленте показываем призыв обновиться. Приложение при этом
    полностью работоспособно — блокировать его нельзя, пока 2.0 не в сторе."""
    try:
        ua = (request.headers.get("user-agent") or "").lower() if request else ""
        if "okhttp" not in ua and "expo/" not in ua:
            return response
        banner = {
            "title": "Вышла F1 Hub 2.0 — обновитесь в RuStore",
            "preview": "Новый дизайн, прогнозы, трансляции и Зал славы. "
                       "Откройте RuStore, найдите F1 Hub и нажмите «Обновить».",
            "url": "https://www.rustore.ru/catalog/app/ru.leadseek.f1hub",
            "date_text": "",
            "photo": "",
        }
        out = dict(response)
        out["posts"] = [banner] + list(response.get("posts") or [])
        return out
    except Exception:
        return response
...
@app.get("/api/news")
async def get_news(request: Request = None):
    ...
    if cached:
        return _with_legacy_upgrade_banner(cached, request)
    ...
    return _with_legacy_upgrade_banner(response, request)
...
async def list_broadcasts(request: Request, race_round: Optional[int] = None, season: Optional[int] = None, is_live: Optional[bool] = None):
    ...
    data = {"broadcasts": _use_mirrored_thumbs(rows)}
    if _is_legacy_app(request):
        # LEGACY_WALL_BROADCASTS: первой записью — переход в RuStore.
        # Поля те же, что у обычной трансляции, чтобы приложение её отрисовало.
        return _wall_list(data, "broadcasts", {
            "id": 0, "race_round": 0, "season": CURRENT_SEASON, "session_type": "review",
            "title": UPDATE_TITLE + " — " + UPDATE_TEXT,
            "video_url": RUSTORE_URL, "embed_url": RUSTORE_URL,
            "is_live": 0, "started_at": None, "ended_at": None,
            "created_by": 0, "created_at": None, "thumbnail_url": None,
        })
    return data
